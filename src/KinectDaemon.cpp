...
std::shared_ptr<PlayCommand> KinectDaemon::play(const std::string &_filename)
{
  ...
  _play->server_address("127.0.0.1");
	...
	std::shared_ptr<std::thread> _thr = std::make_shared<std::thread>(&KinectDaemon::execute, this , ZMQMessageType::PLAY,_play);
	_thr->detach();
...
std::shared_ptr<RecordCommand> KinectDaemon::record()
{
  ...
  _record->num_kinect_cameras(4);
	...
	std::shared_ptr<std::thread> _thr = std::make_shared<std::thread>(&KinectDaemon::execute, this , ZMQMessageType::RECORD,_record);
	_thr->detach();
